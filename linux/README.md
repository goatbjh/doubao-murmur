...
>
> **GoatBJH 维护分支**：基于上游 `v1.5.1`，额外修复 evdev 设备断开后的单核忙循环、输入设备热插拔恢复、WebSocket 正常 EOF 状态清理和 asyncio event loop 释放。
...
GoatBJH 维护分支会在 evdev 设备 EOF、断开或持续报错时关闭失效 fd，并每两秒重新扫描支持按键事件的设备。若仍出现高 CPU，请先保留进程并采集线程状态：
